...
{
    public static bool LockedUI { get; set; } = true;
    ...
    static Dictionary<LogType, int> logTypesCount;

    public static event System.Action OnLogAdded;

    public static int GetLogTypeCount(LogType type, int max = 99)
    {
        var count = logTypesCount[type];
    ...
    void InitLogTypesCount()
    {
        ///
        if (logTypesCount == null)
        {
            logTypesCount = new Dictionary<LogType, int>();
        }

        ///
        foreach (var item in System.Enum.GetValues(typeof(LogType)))
        {
            logTypesCount[(LogType)item] = 0;
        }
    }
